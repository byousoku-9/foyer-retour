...
Preuve locale disponible : `python -m server.evals.run --suite parsing --profile full --max-cost 0.01` compare les onze transcriptions avec un coût fournisseur nul. Le résultat local attendu est volontairement rouge : 8 `bonne_reponse` et 3 `parsing` sur les colonnes Baloise p30/p40/p48. Le code de sortie est donc 1, ce qui publie honnêtement la divergence sans en faire un incident.
