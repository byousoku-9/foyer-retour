...
≈ 0,67 € à froid ; le plafond par requête passe à 1,30 €, puis à 3,50 €. L'écart tient au préfixe : depuis que le
...
## Story 5.6 (T20) — un gate compare ce qui change une réponse, pas ce qui règle l'exploitation

Trois déploiements verts au job « verifier » ont été refusés à la promotion par le smoke avec `gate_perime` sur les trois documents : le contexte de gate comparait les 180 seuils publiés, dont trois interrupteurs d'exploitation qui diffèrent *par construction* entre le poste qui mesure un gate et l'image de production — l'audit exact (jamais écrit sur un disque partagé), le maintien des préfixes au chaud (posé par `deploy.yml`, là où `--min-instances=1` est décidé) et le budget de la campagne. `/api/v1/sante` publie toujours `thresholds()` en entier, mais le gate ne compare plus que le sous-ensemble « pipeline » (`Settings.gate_thresholds`), selon une règle écrite dans `config.py` et tenue par un témoin : un seuil entre dans le contexte de gate s'il peut changer une claim, un verdict ou une citation — sinon c'est un interrupteur d'exploitation, qui décide de ce que le service dépense, garde ou refuse, jamais de ce qu'il répond.

## Story 5.6 (L1) — deux limites que la forme lisible rend visibles

Rendre la réponse explicative — une réponse par sous-question, les clauses citées lisibles sous chaque paragraphe, l'avancement diffusé pendant le calcul — fait apparaître ce que le produit ne fait pas, et il vaut mieux le nommer que le laisser chercher. **Pas de question libre sur un sinistre en cours** : le fil de l'outil sinistre n'est pas un dialogue mais un *registre de faits*, qui n'accepte que des réponses aux questions que le verdict a posées, les enregistre dans un état signé et recalcule le verdict par la même table, sans appeler de modèle ; « et si le voisin refuse ? » n'y a pas de place, parce qu'un tour de dialogue libre rouvrirait la porte que l'epic entier ferme — une réponse qui n'est adossée à aucune clause relue. **Pas de complément web** : le guide répond sur l'édition ingérée du site et sur rien d'autre, et quand une question porte sur ce que les fiches ne traitent pas — les démarches du côté français, par exemple — la réponse le dit en une ligne et s'arrête là, plutôt que d'aller chercher ailleurs une phrase sans source citable, sans fraîcheur vérifiable et sans garantie de rester vraie demain. Les deux limites sont assumées, et surtout **dites à l'utilisateur** plutôt que masquées par une réponse plausible.
