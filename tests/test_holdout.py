...
def _isolation() -> dict[str, object]:
    return {
        "sandbox": "sandbox-exec+provider-relay",
        "filesystem_probes": {
            "generator_stage": True,
            "generator_product_code": False,
            "generator_cases_a": False,
            "generator_parent": False,
            "validator_product_code": True,
            "validator_corpora": False,
            "sealer_parent": False,
            "sealer_product": False,
            "sealer_generator_runtime": False,
        },
        "egress": {
            "allowed_hosts": ["chatgpt.com"],
            "provider_via_relay": True,
            "public_repository_via_relay": False,
...
def _lot() -> holdout.LotScelle:
    cases = [
        ...
        *({"doc_id": "document-neutre", "case": _case(f"sinistre-neutre-{index}", "sinistre")} for index in range(1, 4)),
    ]
...
def _public(tmp_path: Path) -> tuple[Path, Path, Path, dict[str, object]]:
    ...
    receipt = {
        ...
        "payload_hmac": "f" * 64,
        "arm_token_digest": hashlib.sha256(hmac.new(SECRET.encode(), b"holdout-c/arm-token/v1", hashlib.sha256).digest()).hexdigest(),
...
def test_un_lot_refuse_multilingue_sans_controle_de_retraduction() -> None:
    raw = _lot().model_dump(mode="json")
    raw["cases"][0]["case"]["famille"] = "multilingue"
    with pytest.raises(ValueError, match="multilingue"):
        holdout.LotScelle.model_validate(raw)
...
def test_les_trois_preuves_arment_puis_lunique_prise_est_irreversible(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for index, condition in enumerate(holdout.CONDITIONS):
        evidence = tmp_path / f"preuve-{index}.txt"
        evidence.write_bytes(f"preuve synthétique {index}".encode())
        path = tmp_path / f"attestation-{index}.json"
        path.write_bytes(
            holdout.json_canonique(
                {
                    "schema_version": 1,
                    "condition": condition,
                    "completed": True,
...
def test_une_attestation_est_liee_aux_octets_de_sa_preuve(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    receipt, lock, _, _ = _public(tmp_path)
    monkeypatch.setattr(holdout, "_secret_keychain", lambda _receipt: SECRET)
    attestations = []
    for index, condition in enumerate(holdout.CONDITIONS):
        evidence = tmp_path / f"preuve-{index}.txt"
        evidence.write_bytes(f"preuve {index}".encode())
        path = tmp_path / f"attestation-{index}.json"
        path.write_bytes(
            holdout.json_canonique(
                {
        ...
        attestations.append(path)
    (tmp_path / "preuve-1.txt").write_text("preuve altérée", encoding="utf-8")
    with pytest.raises(holdout.RefusHoldout, match="différent de l'artefact"):
        holdout.armer(receipt_path=receipt, lock_path=lock, attestations=attestations)
    assert holdout.lire_verrou(lock).state == "sealed"


def test_le_chiffrement_authentifie_dechiffre_fidelement() -> None:
...
def test_executer_revoque_la_cle_avant_consumption_et_transmet_le_lot_memoire(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    receipt_path, lock_path, payload_path, receipt = _public(tmp_path)
    lot_raw = _lot().model_dump(mode="json")
    receipt["cases_hash"] = holdout.digest_json(sorted(lot_raw["cases"], key=lambda item: item["case"]["id"]))
    receipt_path.write_bytes(holdout.json_canonique(receipt) + b"\n")
    ...
    def secret_while_armed(_receipt: holdout.RecuPublic) -> str:
        observed["state_at_key"] = holdout.lire_verrou(lock_path).state
        return SECRET

    def delete_while_armed(_receipt: holdout.RecuPublic) -> None:
        observed["state_at_delete"] = holdout.lire_verrou(lock_path).state
    ...
    monkeypatch.setattr(holdout, "_secret_keychain", secret_while_armed)
    monkeypatch.setattr(holdout, "_supprimer_secret_keychain", delete_while_armed)
    ...
    assert observed["state_at_key"] == "armed"
    assert observed["state_at_delete"] == "armed"
    assert observed["argv"] == ["--profile", "full"]
    assert len(observed["lot"].cases) == 4
    assert holdout.lire_verrou(lock_path).arm_token is None
    assert json.loads((tmp_path / "consumed.marker").read_text())["kind"] == "secret-hmac-sha256-v1"


def test_supprimer_la_marque_et_rearmer_ne_permet_jamais_un_second_run(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    receipt_path, lock_path, payload_path, receipt = _public(tmp_path)
    lot_raw = _lot().model_dump(mode="json")
    receipt["cases_hash"] = holdout.digest_json(sorted(lot_raw["cases"], key=lambda item: item["case"]["id"]))
    receipt_path.write_bytes(holdout.json_canonique(receipt) + b"\n")
    arm_token = hmac.new(SECRET.encode(), b"holdout-c/arm-token/v1", hashlib.sha256).hexdigest()
    armed = json.loads(lock_path.read_text("utf-8")) | {
        "receipt_digest": holdout.digest_json(receipt),
        "state": "armed",
        "conditions": {condition: True for condition in holdout.CONDITIONS},
        "armed_at": "2026-09-01T00:00:00+00:00",
    ...
    lock_path.write_bytes(holdout.json_canonique(armed) + b"\n")
    key_available = True
    runner_calls = 0

    def get_secret(_receipt: holdout.RecuPublic) -> str:
        if not key_available:
            raise holdout.RefusHoldout("clé one-shot indisponible ou interaction utilisateur refusée")
        return SECRET

    def delete_secret(_receipt: holdout.RecuPublic) -> None:
        nonlocal key_available
        key_available = False

    def run_once(_argv: list[str], *, lot_cases_fourni: object) -> int:
        nonlocal runner_calls
        assert lot_cases_fourni is not None
        runner_calls += 1
        return 0

    monkeypatch.setattr(holdout, "_secret_keychain", get_secret)
    monkeypatch.setattr(holdout, "_supprimer_secret_keychain", delete_secret)
    monkeypatch.setattr(holdout, "dechiffrer", lambda *_args: holdout.json_canonique(lot_raw))
    monkeypatch.setattr(holdout.runner, "main", run_once)
    assert (
        holdout.executer(
            receipt_path=receipt_path,
            lock_path=lock_path,
    ...
    (tmp_path / "consumed.marker").unlink()
    lock_path.write_bytes(holdout.json_canonique(armed) + b"\n")
    with pytest.raises(holdout.RefusHoldout, match="clé one-shot indisponible"):
        holdout.executer(
            receipt_path=receipt_path,
            lock_path=lock_path,
            payload_path=payload_path,
...
def test_un_verrou_consomme_exige_les_conditions_et_aucun_jeton() -> None:
    base = {
        "schema_version": 1,
        "receipt_digest": "a" * 64,
        "state": "consumed",
    ...
    holdout.VerrouPublic.model_validate(base)
    with pytest.raises(ValueError, match="conditions vraies"):
        holdout.VerrouPublic.model_validate(
            base
            | {
                "conditions": {condition: False for condition in holdout.CONDITIONS},
            }
        )
    with pytest.raises(ValueError, match="sans tentative ni jeton"):
        holdout.VerrouPublic.model_validate(base | {"arm_token": "b" * 64})
