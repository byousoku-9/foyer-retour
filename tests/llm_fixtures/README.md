...
Un fichier `{module}.{nom_du_test}.json` par test qui appelle un modèle, écrit par `tests/fixtures.py` (`LLMRecorder`).
...
Les fichiers sont committés : pour ré-enregistrer, supprimer le JSON et relancer le test avec la clé ; la régénération est un commit qui dit pourquoi.
