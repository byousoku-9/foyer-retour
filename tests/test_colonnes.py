...
def _page_deux_colonnes(*, tete: list[p.PageLine] | None = None, page: int = 1) -> p.PageText:
    lines = [*(tete or []), *_colonne("G", GAUCHE_X), *_colonne("D", DROITE_X)]
...
def test_deux_colonnes_trop_courtes_ou_trop_peu_peuplees_ne_sont_pas_retenues() -> None:
    ...
    peu_peuple = p.PageText(page=1, width=595, height=842, lines=[
    ...
    trop_courte = p.PageText(page=1, width=595, height=842, lines=[
...
def test_une_gouttiere_plus_etroite_que_le_seuil_nest_pas_une_colonne() -> None:
    serree = p.PageText(page=1, width=595, height=842, lines=[
...
def test_le_seuil_de_gouttiere_est_un_reglage_publie_et_non_une_valeur_en_dur(
        monkeypatch: pytest.MonkeyPatch) -> None:
    serree = p.PageText(page=1, width=595, height=842, lines=[
        *_colonne("G", GAUCHE_X, n=6), *_colonne("D", 266.0, n=6),
    ])
...
def test_un_titre_traversant_ouvre_une_bande_lue_avant_ses_colonnes() -> None:
    ...
    page = p.PageText(page=1, width=595, height=842, lines=[*haut, titre, *bas])
    p.ordonner_pages([page])
...
def test_le_vocabulaire_du_corpus_synthetique_est_neutre() -> None:
    ...
    source = (inspect.getsource(_ligne) + inspect.getsource(_colonne)
              + inspect.getsource(_page_deux_colonnes)).lower()
