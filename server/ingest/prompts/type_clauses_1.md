...
- `article_refs`: numéros décimaux pointés explicitement visés par un renvoi (ex. `2.4`), jamais des `block_id`.
- `scope_articles`: numéros d'articles auxquels la clause dit explicitement limiter sa portée. Une plage peut être écrite `2.4.1-2.4.3`; le code la développera sous sa borne. Vide signifie le sous-arbre de son propriétaire.
