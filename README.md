...
- **Il se teste sans navigateur** : toute la logique vit dans `web/app/chat.js`, en fonctions pures ; `tests/js/chat_cases.mjs` le charge dans un `node:vm` avec `window`, `location` et `fetch` doublés, et `tests/test_web_chat.py` asserte les relevés. Aucune dépendance ajoutée ; sans `node`, ces cas passent en `skip`. La peinture, elle, est vérifiée en live (`docs/tests-live.md`).
